...
const bird = {
    ...
    update: function(){
        //IF THE GAME STATE IS READY, THE BIRD MUST FLAP SLOWLY
        ...
        if(state.current == state.getReady){
            this.y = 150; //reset the position of bird after gameOver
            ...
            //If the speed is greater than the jump, then the bird is falling down.
...
//GET READY MESSAGE TO BEGIN GAME
const getReady = {
    sX: 0,
    sY: 228,
    w: 173,
...
const pipes = {
    ...
    update: function(){
        ...
        for(let i = 0; i < this.position.length; i++){
            ...
            //if Pipes go outside of canvas, we must delete them from the array
            if(p.x + this.w <= 0){
                this.position.shift();
                score.value += 1;
                SCORE_S.play();
